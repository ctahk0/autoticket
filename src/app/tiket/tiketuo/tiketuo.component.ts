...
@Component({
  selector: 'app-tiketuo',
  templateUrl: './tiketuo.component.html',
  styleUrls: ['./tiketuo.component.css'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class TiketUOComponent implements OnInit {
    ...
    getCurrentOdds(tip: string, uo25u: number, uo25o: number) {
        return (tip === 'U' ? uo25u : uo25o);
    }
    ...
    oddsSum() {
        ...
        this.tiket.forEach(el => {
            sumo += Number(el.odds);
            sumc += (el.tip === 'U' ? Number(el.cuo25_u) : Number(el.cuo25_o));
